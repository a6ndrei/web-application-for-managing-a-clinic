...
router.get("/all", verifyToken, async (req, res) => {
  if (req.userRol !== "admin") {
    return res.status(403).json({
      message:
        "Acces interzis. Doar administratorii pot vedea toate programările.",
    });
  }
...
router.post("/book", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      return res
        .status(400)
        .json({ message: "Acest interval orar este deja ocupat." });
    }
...
router.get("/recommend-slots", async (req, res) => {
  ...
  try {
    ...
    const recommended = ALL_TIMES.filter((t) => !busyTimes.includes(t))
      .map((t) => ({
        time: t,
        score: statsMap[t] || 0,
      }))
