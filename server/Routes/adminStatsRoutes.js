...
const isAdmin = (req, res, next) => {
  if (req.userRol !== "admin") {
    return res
      .status(403)
      .json({
        message: "Acces interzis. Doar administratorii pot vedea statistici.",
      });
  }
  next();
};
...
router.get("/predictions", verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    appointments.forEach((a) => {
      countsPerDay[a.data_programare] =
        (countsPerDay[a.data_programare] || 0) + 1;
    });
    ...
    const weekdayWeights = weekdayCounts.map((c) =>
      totalWeekdays > 0 ? c / totalWeekdays : 1 / 7,
    );
